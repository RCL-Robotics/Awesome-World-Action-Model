...
export const papers = (rawPapers as Paper[]).toSorted((a, b) => b.submittedDate.localeCompare(a.submittedDate) || b.id.localeCompare(a.id));
export { meta };
...
export const categoryFor = (name: string) => categories.find(c => c.name === name);
export const formatDate = (date: string, options: Intl.DateTimeFormatOptions = { day: 'numeric', month: 'short', year: 'numeric' }) => new Intl.DateTimeFormat('en-GB', { ...options, timeZone: 'UTC' }).format(new Date(`${date.slice(0, 10)}T00:00:00Z`));
