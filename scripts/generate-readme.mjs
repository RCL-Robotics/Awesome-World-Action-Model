...
const repository = 'https://github.com/RCL-Robotics/Awesome-World-Action-Model';
...
export function renderReadme(inputPapers, inputMeta) {
  ...
  const lines = [
    ...
    'The catalog combines Notion classifications with explicit local category reviews. WAM Components groups reusable encoders, language and generative backbones, world predictors, spatial perception, action policies, and training methods. Foundational work retains theory, planning, surveys, and classic world-model research. Reviewed local arXiv additions retain their original-source metadata and reading-report summaries, with unassigned classifications kept separate. Missing abstracts and other unrecorded information remain unfilled. A publication year is kept separate from a submission date.', '',
    '## Major categories', '',
  ...
  lines.push(
    ...
    'Notion is the editorial source. Reviewed local additions are retained in `data/local-papers.json` and merged during export. A Notion record with the same ID takes precedence over local additions; local-only records remain in the catalog. Explicit component placements in `data/classification-overrides.json` are applied after this merge, changing only the major category and component subcategory. The versioned catalog in `data/papers.json` is shared by the website and this README. Builds use the committed catalog and need no Notion credentials.', '',
    ...
    'The exporter reads every result page, retries transient failures, preserves all rich-text fragments, and exports only the 25 explicit catalog fields, including six recorded taxonomy fields. It does not export Notion classification evidence, retrieval logs, the internal Date field, private paths, or raw page metadata. Synchronization is read-only with respect to Notion and does not infer classifications. It reapplies accepted component reviews; source verification status, quadrant assignments, and reading-time report snapshots are preserved.', '',
