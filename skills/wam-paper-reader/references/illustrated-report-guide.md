...
For an HTML technical resource only (`resourceType: "technical-resource"`, `reportStatus: "resource-reviewed"`, first source `kind: "html"`), a nonempty `sourceOrganization` may replace `authors` when the original inspected identity section visibly names the organization in its source/publisher/rightsholder role and no individual byline is stated. The website labels this credit **Source organization**. Do not infer authorship from a publisher, copyright, logo or website name. Use the exact visibly supported organization name, a nonempty `location` equal to the identity section ID, and the primary `sourceSha256`; omit `authors`, `affiliations` and `page` entirely. `authors` and `sourceOrganization` are mutually exclusive. Explicitly disclose the absent individual byline and the credit basis in `report.coverage.omissions`. A copyright year is not a publication date. The fresh independent reviewer must verify the declared credit role from that same original identity image; an unsupported name or role fails identity and approval. All existing source, image, hash, receipt and scientific checks remain required.
...
## Explicit Microsoft original MP4 route

Only the source-specific `config.mp4Original.policyVersion === "wam-original-mp4-full-report-v1"` normal coordinator route permits these original HTML-linked MP4 stills. It is an explicit exception to the unavailable-PDF instruction above; do not infer this route for other sources. Read every retained primary-text chunk. The fixed reviewer context contains that complete primary text, all report/edition fields, all 40 bound original native PNG samples (four per clip), and every selected public full-frame PNG. Copy exact pixels and the supplied `mp4Source` locator, source URL and rendering disclosure; do not use PDF page/crop, `htmlSource`, GIF or screenshot fields. Each asset path must be `report-assets/<paperId>/<visualId>.png`. Metadata uses `location: "complete-primary-text"`, with the verified title/source hash and no invented PDF page.

This article is a resource overview, with no source equations or quantitative benchmark table. Attribute original captions and author claims. Samples establish only visible content at recorded actual times; they do not establish continuous motion, audio, latency, benchmark success, autonomy or model execution. Preserve plug-insertion human guidance, work-in-progress adaptation and the prompt-free B-roll boundary. A fresh independent complete-report review and every required image verdict remain mandatory; source preparation or an old receipt is not acceptance.


### Selected author HTML book nodes

The explicit `wam-selected-original-html-evidence-v1` profile binds Sutton and Barto's author-maintained first-edition archive separately from the selected-text hash and 41 original node hashes. It is selected Chapters 1, 3, 4 and 9 plus bibliography, not a PDF or whole-book reading. Keep one primary book source; a visual's locator supplies its exact original node URL, anchor, character range and native PNG uses. Read every selected text chunk. Use the source helper's `--catalog` to find original regions, `--show REGION_ID` for region images and `--asset ORIGINAL_NAME.png` for unchanged native PNG inspection. Delivery is not viewing. Keep exact actual reading/viewing scope.

The final reviewer must receive the author identity region and every region needed by report evidence, plus the chosen original visuals and their paired source context. Acquisition of 734 files or the earlier reader's 343 scientific PNG views never substitutes for this review. The four missing footnote3.3 PNGs263–266 stay explicit; do not infer their mathematics. The source parser does not redraw equations. Typography/reflow is a disclosed wrapper, not a pixel-identical website screenshot. If used evidence remains unreadable, fail and preserve the issue; no blanket legibility flag or prior-source waiver is permitted.

Future acceptance requires the fresh normal receipt with four exact identity enumerations, every attachment's three truthful flags, current source/descriptor/context/reference bytes, and the distinct selected-original-HTML rerender proof. Legacy PDF, HTML, Helix, OpenScene or identity-only proofs cannot replace it.
